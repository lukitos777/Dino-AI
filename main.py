...
# assets for dinosurs 
skins = [
    "default", "aqua", "black", "bloody", "cobalt", "gold", "insta",
    "lime", "magenta", "magma", "navy", "neon", "orange", "pinky",
    "purple", "rgb", "silver", "subaru", "sunny", "toxic"
]
names = [
    "Флафи", "Фалафель", "Ведьмак", "Лютик", "Пучеглазик", "Слайм", "Шустрый", "Следопыт",
...
class Dino:
    ...
    def __init__(self, x: int, y: int, color: str = "default", name: str = None) -> None:
        self.color = color
        self.load_sprites()
        self.hitbox = pygame.Rect( # hit-box for dino
            x, y, 
            self.sprites["run"][0].get_width(), 
    ...
    def load_sprites(self):
        self.sprites["jump"].append( # creating animations
    ...
    def update(self): # changing state of dino
...
def run_game(genomes, config):
    ...
    while True:
        ...
        if len(enemies) < 3:
            enemies.append(
                Cactus(enemies[len(enemies) - 1].hitbox.x + width 
